...
for file in config_files[2:]:
    ...
    cluster_map = [[2*i, 2*i+1] for i in range(int(dim/2))]
    ...
    expr_name = 'cpf_{}_pc_{}_s_{}'.format(config_id, prior_cov, shift)
    cc = cf.ConfigCollector(expr_name = expr_name, folder = str(script_dir) + '/results_c')
    ...
    pf = cpf.ClusterPF(index_map, cluster_map, model, particle_count = 50, folder = cc.res_path)
