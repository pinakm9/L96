...
print(configs)
#np.zeros((num_fl, ev_time))
for i in range(len(configs)):
    config_id_1 = configs[i].split('_')[1][:-5]
    for j in range(i+1, len(configs), 1):
        config_id_2 = configs[j].split('_')[1][:-5]
        data = {'time': [], 'fl_id':[], 'Wasserstein_2': []}
        for fl in range(num_fl):
            assml_file_1 = expr_folder + '/fl_{}_config_{}.h5'.format(fl, config_id_1)
            assml_file_2 = expr_folder + '/fl_{}_config_{}.h5'.format(fl, config_id_2)
            print('comparing {} vs {} for filter realization {} ...'.format(config_id_1, config_id_2, fl))
            pf_comp = cd.PFComparison(assml_file_1, assml_file_2)
            data['time'] += list(range(ev_time))
            data['fl_id'] += [fl] * ev_time 
