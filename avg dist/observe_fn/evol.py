...
for file in config_files[:1]:
    ...
    ev_time = 10 #config["Number of assimilation steps"]
    ...
    model, gen_path = Lorenz96_fn.get_model(x0=x0, size=ev_time, prior_cov=prior_cov, obs_cov=obs_cov,  shift=shift, obs_gap=obs_gap, obs_n=10)
    ...
    pf = lpf.LocalPF(0.7, 3, 1.0, model, particle_count = 200, record_path = cc.res_path + '/assimilation.h5')
    ...
    if pf.status == 'success':
        ...
        pf.plot_trajectories(hidden_path, coords_to_plot=[25, 30, 35, 39], file_path=cc.res_path + '/trajectories.png', measurements=False)
