...
for file in config_files[:2]:
    ...
    for seed in seeds[:1]:
        ...
        cc = cf.ConfigCollector(expr_name = expr_name, folder = str(script_dir) + '/results_b')
        ...
        pf = fl.ParticleFilter(model, particle_count = particle_count, folder = cc.res_path)
        pf.update(observed_path, method = 'mean', resampling_method='systematic_noisy', threshold_factor=1.0)
